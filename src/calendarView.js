  ...
  render: function () {
    ...
    if (component.state.printAll === true) {
      eventList = this.props.events.map(function (event, index, events) {
        return (<Event event={event} isLast={index === (events.length - 1)} key={index}/>);
      });
    } else {
      for (var i = 0, end = component.props.displayLength; i < end; i += 1) {
        if (this.props.events[i]) {
          eventList.push(<Event event={this.props.events[i]}
...
var Calendar = React.createClass({
  ...
  render: function () {
    ...
    if (this.state.windowWidth > 480) {
      ret = (<DesktopCalendar events={events} beginningDay={beginningDay} endingDay={endingDay} dates={this.state.dates} dateRange={this.state.dateRange} />);
    } else {
      ret = (<MobileCalendar events={events}  beginningDay={beginningDay} endingDay={endingDay} />);
    }
    return ret;
  }
});
