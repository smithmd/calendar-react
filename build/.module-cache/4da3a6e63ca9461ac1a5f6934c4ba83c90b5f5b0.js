...
var EventCalendar = React.createClass({displayName: "EventCalendar",
  ...
  filterArray: function (comp, array) {
    var show = true;
    if (array.indexOf('Any') < 0 && array.length > 0) {
      var showDivision = false;
      for (var i = 0; i < array.length; i++) {
        if (comp && ~comp.indexOf(array[i])) {
          showDivision = true;
          break;
        }
      }
  ...
  render: function () {
    ...
    var events = this.state.data.filter(function (event) {
      ...
      // return if we know we already don't want this event
      if (component.filterArray(event.divisions, component.state.divisions) === false) return false;
