...
var EventDate = React.createClass({displayName: "EventDate",
  render: function () {
    var component = this;
    var remaining = component.props.displayLength;
    var eventList = this.props.events.map(function (event, index, events) {
      if (component.props.printAll =='true') {
        ...
        var last = index === (component.props.displayLength - 1);
        return (React.createElement(Event, {event: event, isLast: last}));
      }
    });
    return (
...
var EventCalendarRow = React.createClass({displayName: "EventCalendarRow",
  render: function () {
    ...
    for (var i = 0; i < 7; i++) {
      ...
      if (day != null)
        days.push(React.createElement(EventDate, {events: events, printAll: "false", displayLength: "3", day: day.date(), key: key, isCurr: day.month() == this.props.month}));
