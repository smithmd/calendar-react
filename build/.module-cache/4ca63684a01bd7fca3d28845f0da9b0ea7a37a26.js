...
var PickList = React.createClass({displayName: "PickList",
  ...
  handleClickChild: function (i, filter) {
    ...
    var tmp = [];
    for (var it = 1; it < arr.length; it++) {
      tmp.push(this.props.data[arr[it]]);
    }
    filter.onNext({venues:tmp});
  },
  render: function () {
    var component = this;
    var list = null;
  ...
  render: function () {
    return (
        React.createElement("span", null, 
          ...
          React.createElement(PickList, {data: campDivisions, category: "division", title: "Camp Divisions", filter: null}), 
          React.createElement(PickList, {data: artsAreas, category: "area", title: "Arts Area", filter: null})
        )
    );
  }
});
