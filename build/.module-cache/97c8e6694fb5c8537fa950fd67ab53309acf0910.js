...
var EventDateList = React.createClass({displayName: "EventDateList",
  ...
  componentDidMount: function () {
    this.loadEvents();
    var component = this;
    window.selectedDate.subscribe(function (s) {
      component.setState({selDate: s.date});
      React.renderComponent(React.createElement(EventDateList, {url: "json/calendar.json", selDate: component.state.selDate}), document.getElementById('prettyEvents'));
    });
  },
  render: function () {
    var edl = this;
    ...
    var eventDateNodes = eventDateIndex.map(function (date, index) {
      return (
          React.createElement(EventDate, {key: date, date: date, selDate: edl.state.selDate, events: eventsByDates[index]})
      );
    });
    return (
...
React.render(
    React.createElement(EventDateList, {url: "json/calendar.json", selDate: moment().format('YYYY-MM-DD')}),
