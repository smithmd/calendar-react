...
var EventCalendar = React.createClass({displayName: "EventCalendar",
  ...
  getInitialState: function () {
    return {data: [], dateFilteredData: [], dates: {}, filters: {}, venues: [], artsAreas: [], divisions: []};
  },
  componentDidMount: function () {
    ...
    calendarDates.subscribe(function (s) {
      ...
      var beginningDay = moment([s.currentYear, s.currentMonth]);
      beginningDay.day(0);
      // moments are bound to last day of month, so we can assume the 31st will always get us the last day
      var endingDay = moment([s.currentYear, s.currentMonth, 31]);
      endingDay.day(6);
      var events = component.state.data.filter(function (event) {
        var m = moment(event.startDate, 'YYYY-MM-DD');
        return m >= beginningDay && m <= endingDay;
      });

      component.setState({dateFilteredData: events});
    });
  ...
  render: function () {
    ...
    var events = this.state.dateFilteredData.filter(function (event) {
      var show = true;
