...
var EventDateList = React.createClass({displayName: "EventDateList",
  ...
  componentDidMount: function () {
    ...
    window.selectedDate.subscribe(function (s) {
      ...
      React.renderComponent(React.createElement(EventDateList, {url: "json/calendar.json", selDate: this.state.selDate}), document.getElementById('prettyEvents'));
    });
  },
  render: function () {
    var eventDateIndex = [];
    ...
    var eventDateNodes = eventDateIndex.map(function (date, index) {
      return (
          React.createElement(EventDate, {key: date, date: date, selDate: this.state.selDate, events: eventsByDates[index]})
      );
    });
    return (
