...
var EventCalendar = React.createClass({displayName: "EventCalendar",
  ...
  getInitialState: function () {
    return {data: [], dates: {}, selectedData: []};
  },
  componentDidMount: function () {
    ...
    var selectedMonthEvents = this.state.data.filter(function (event) {
      var m = moment(event.startDate, 'YYYY-MM-DD');
      return m >= beginningDay && m <= endingDay;
    });
    component.setState({selectedData: selectedMonthEvents});
  },
  render: function () {
    var component = this;

    var events = this.state.selectedData.filter(function (event) {
      var show = true;
