...
var EventCalendar = React.createClass({displayName: "EventCalendar",
  ...
  render: function () {
    ...
    var events = this.state.data.filter(function (event) {
      ...
      console.log({
        'year': event.startDate.substring(0,4),
        'month': event.startDate.substring(5,7),
        'day':event.startDate.substring(8)
      });
      m.set({
        'year': event.startDate.substring(0,4),
        'month': event.startDate.substring(5,2),
        'day':event.startDate.substring(8,2)
      });
