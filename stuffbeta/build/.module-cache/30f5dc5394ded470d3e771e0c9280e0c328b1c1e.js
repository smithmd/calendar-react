var Calendar = React.createClass({displayName: "Calendar",
  ...
  filterFromSearch: function (eventObj, searchString) {
    var search = searchString.toLowerCase().split(' ');
    var string_exists = false;
    // check various fields
    // include multiple search terms
    if (eventObj) {
      for(var i = 0; i < search.length; i+=1) {
        // prevent page from filtering on all if there's a trailing space
        if (search[i].length > 0) {
          // check if title matches string
          if (eventObj.title) {
            if (eventObj.title.toLowerCase().indexOf(search[i]) > -1
            ) {
              return true;
            }
          }
          ...
          if (eventObj.genre) {
            if (eventObj.genre.toLowerCase().indexOf(search[i]) > -1
            ) {
              return true;
            }
          }
          ...
          if (eventObj.campDivision) {
            if (eventObj.campDivision.toLowerCase().indexOf(search[i]) > -1
            ) {
              return true;
            }
          }
  ...
  filterData: function (component, beginningDay, endingDay) {
    return function (event) {
      ...
      if (component.filterFromSearch(event, component.state.searchText) === false) return false;
